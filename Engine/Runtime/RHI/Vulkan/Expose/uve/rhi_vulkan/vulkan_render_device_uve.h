...
// "Vulkan (B1 bindless + M5b storage images)" only when both optional tiers are live, and
// uses the corresponding B1/M2c or M5b/M2c name otherwise — never "Vulkan" unqualified — so
// logs, editor overlays, and bug reports cannot mistake the current slice for the finished
// backend. IsUsableUVE() reflects the real
