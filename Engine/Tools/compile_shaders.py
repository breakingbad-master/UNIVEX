...
(ESSL 3.10 is selected for compute/SSBO variants); D3D12 consumes HLSL; Metal and iOS consume MSL. The source hash, target defines, and tool versions
...
# The authoring source has two source-semantics branches today. Vulkan-family artifacts use the
# explicit descriptor/push-constant branch; text backends are generated from OpenGL-semantics
# SPIR-V, which SPIRV-Cross can translate to HLSL/MSL without losing nested UBO layout information.
# This is a build-time source policy, not a claim that the runtime text backends use OpenGL binding
# numbers: each native backend receives its own cross-compiled resource declarations.
_VULKAN_TARGETS = frozenset(("vulkan", "android-vulkan"))
_OPENGL_SEMANTICS_TARGETS = frozenset(("opengl", "gles", "d3d12", "metal", "ios"))
...
def _target_command(
    ...
    if target in _VULKAN_TARGETS:
        shutil.copyfile(spirv_path, output_path)
        return None
    if target == "opengl":
        return [spirv_cross, str(spirv_path), "--version", "450", "--output", str(output_path)]
    if target == "gles":
        # The Android fallback context is ES 3.0 for ordinary graphics. Compute and instanced
        # graphics variants use shader-storage buffers, which are an ES 3.1 feature; emit those
        # variants as ES 3.1 instead of claiming an impossible ES 3.0 artifact. The runtime's
        # explicit capability tier selects the non-instanced/source fallback on ES 3.0 rather than
        # attempting to load this higher-floor artifact.
        uses_shader_storage = "UVE_INSTANCED" in defines
        gles_version = "310" if uses_shader_storage or stage in ("comp", "geom", "tesc", "tese") else "300"
        return [
            spirv_cross,
            str(spirv_path),
            "--es",
...
def _compile_target_spirv(
    ...
    # OpenGL-style and Vulkan-style SPIR-V source environments are different. The OpenGL-style
    # path keeps default-block uniforms that the GL runtime updates with glUniform and is also the
    # portable source representation SPIRV-Cross uses for HLSL/MSL nested constant-buffer layouts;
    # the Vulkan path selects the explicit push-constant/descriptor layout used by native Vulkan.
    # Cross-compiling both from the same authoring file is why target-specific defines are part of
    # the artifact manifest.
    uses_opengl_semantics = target in _OPENGL_SEMANTICS_TARGETS
    semantics = "-G" if uses_opengl_semantics else "-V"
    compile_command = [
        glslang,
        semantics,
        "-S",
    ...
    if uses_opengl_semantics:
        # OpenGL-semantics SPIR-V requires explicit locations for non-opaque default-block uniforms.
        # The source remains directly consumable by the runtime compiler without those generated
        # locations; glslang assigns them only for these offline text/cross-backend artifacts.
...
def compile_shader(
    ...
    for target in target_list:
        ...
        command = _target_command(
            target, stage, target_define_list, spirv_cross, spirv_by_target[target], artifact_path
        )
    ...
    return {
        ...
        "source_fnv1a64": _fnv1a64(source_bytes),
        "stage": stage,
        "entry_point": entry,
...
def main(argv: list[str]) -> int:
    ...
    manifest = {
        # Format 3 adds the runtime-verifiable FNV-1a source fingerprint.  The SHA-256 remains
        # the operator-facing provenance field; the compact fingerprint avoids bringing a second
        # crypto library into the shipped shader manager just to reject stale cooked output.
        "format": 3,
        "toolchain": {
